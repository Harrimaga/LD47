...
namespace LD47.Ships
{
    public abstract class EnemyPlane : Plane
    {
        ...
        {
            Globals.currentLevel.projectiles.Add(new Weapons.Projectile(this, new Vector2(projectileSpeed*(float)Math.Sin(rotation), -projectileSpeed * (float)Math.Cos(rotation)), position + new Vector2(w / 2, h / 2) - new Vector2(6, 12), 12, 24, 4));
        }
